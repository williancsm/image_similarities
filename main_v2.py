...
for i in range(len(arr)):
  ...
  for j in range(i + 1, len(arr)):
    ...
    if ssim_value > 0.6:
      if not os.path.exists('moved/img_%d' % i):
        os.mkdir('moved/img_%d' % i)
      
      if found_sim:
        os.rename('images/%s' % img_a_path, 'moved/img_%d/%s' % (i, img_a_path))
        found_sim = False    

      os.rename('images/%s' % img_b_path, 'moved/img_%d/%s' % (i, img_b_path))
      os.remove('preprocess/%s' % img_b_path) 
      arr[j] = 'moved'
      
'''
    if ssim_value >= 0.6 and ssim_value < 0.7:
      os.mkdir('moved/60_70/img_%d' % i)
      os.rename('images/%s' % img_b_path, 'moved/60_70/img_%d/%s' % (i, img_b_path))
      os.rename('images/%s' % img_a_path, 'moved/60_70/img_%d/%s' % (i, img_a_path))
      os.remove('preprocess/%s' % img_b_path) 
      arr[j] = 'moved'
    
    if ssim_value >= 0.7 and ssim_value < 0.8:
      os.mkdir('moved/70_80/img_%d' % i)
      os.rename('images/%s' % img_b_path, 'moved/70_80/img_%d/%s' % (i, img_b_path))
      os.rename('images/%s' % img_a_path, 'moved/70_80/img_%d/%s' % (i, img_a_path))
      os.remove('preprocess/%s' % img_b_path)
      arr[j] = 'moved'
    
    if ssim_value >= 0.8 and ssim_value < 0.9:
      os.mkdir('moved/80_90/img_%d' % i)
      os.rename('images/%s' % img_b_path, 'moved/80_90/img_%d/%s' % (i, img_b_path))
      os.rename('images/%s' % img_a_path, 'moved/80_90/img_%d/%s' % (i, img_a_path))
      os.remove('preprocess/%s' % img_b_path)
      arr[j] = 'moved'
      
    if ssim_value >= 0.9 and ssim_value <= 1.0:
      os.mkdir('moved/90_100/img_%d' % i)
      os.rename('images/%s' % img_b_path, 'moved/90_100/img_%d/%s' % (i, img_b_path))
      os.rename('images/%s' % img_a_path, 'moved/90_100/img_%d/%s' % (i, img_a_path))
      os.remove('preprocess/%s' % img_b_path)
      arr[j] = 'moved'
'''    
